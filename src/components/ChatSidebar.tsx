...
const ChatSidebar = ({
  ...
  if (isCollapsed) {
    return <div className="w-16 border-r border-border bg-card flex flex-col items-center py-4 space-y-4">
        ...
        <Button variant="ghost" size="sm" onClick={handleLogout} className="w-10 h-10 p-0">
          <LogOut className="h-5 w-5" />
        </Button>
      </div>;
  }
  return <div className="w-64 border-r border-border bg-card flex flex-col h-full sticky top-0">
      {/* Header */}
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="sm" onClick={handleToggleView} className={`w-8 h-8 p-0 ${isOnContracts ? 'bg-muted text-primary' : ''}`} aria-label={isOnContracts ? "Back to chat" : "View contracts"} title={isOnContracts ? "Back to Chat" : "View Contracts"} disabled={!currentConversation}>
        ...
        {loading ? <div className="space-y-2">
          ...
          return <div key={key}>
                  ...
                  <div className="space-y-1">
                    {groupConversations.map(conversation => <div key={conversation.id} className={`group flex items-center gap-2 p-2 rounded-lg cursor-pointer hover:bg-muted/50 transition-colors ${currentConversation?.id === conversation.id ? 'bg-muted' : ''}`} onClick={() => handleConversationClick(conversation)}>
                        <FileText className="h-4 w-4 text-muted-foreground shrink-0" />
                        
                        {editingId === conversation.id ? <input value={editingTitle} onChange={e => setEditingTitle(e.target.value)} onBlur={handleEditSubmit} onKeyDown={e => {
                  if (e.key === 'Enter') handleEditSubmit();
                  if (e.key === 'Escape') handleEditCancel();
                }} className="flex-1 bg-transparent border-none outline-none text-sm" autoFocus /> : <span className="flex-1 text-sm truncate">
                            {conversation.title}
                          </span>}
